...
async function scan({ forceReparse = false } = {}) {
  ...
  const [ftCalls, erasmusCalls, interregCalls, creativeCalls, esfCalls] = await Promise.all([
  ...
  const allRaw = [...ftCalls, ...erasmusCalls, ...interregCalls, ...creativeCalls, ...esfCalls].filter(c => {
